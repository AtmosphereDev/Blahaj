	...
	public void appendHoverText(ItemStack itemStack, TooltipContext tooltipContext, List<Component> consumer, TooltipFlag tooltipFlag) {
		...
		if(itemStack.has(DataComponents.CUSTOM_DATA)) {
			...
			if(itemStack.has(DataComponents.CUSTOM_NAME)) {
				consumer.add(Component.translatable("tooltip.blahaj.owner.rename", this.getDescriptionId(), Component.literal(owner)).withStyle(ChatFormatting.GRAY));
			}
			else {
				consumer.add(Component.translatable("tooltip.blahaj.owner.craft", Component.literal(owner)).withStyle(ChatFormatting.GRAY));
			}
		}
	}
